# из файла typing берут константу (TYPE_CHECKING),переменную Any,которая проходит через класс обжект, но что происходит в классе я не понимаю и функцию cast 
#  и что делает эта функция я тоже не могу понять, из описания только прочитал, что функция переводит что-то в какой-то тип, но это мне не посильно
from typing import (
    TYPE_CHECKING,
    Any,
    Callable,
...
# импортирует файл warning, в котором есть функции действующие по принципу создания переменной по возврату сообщения о предупреждении
import warnings
